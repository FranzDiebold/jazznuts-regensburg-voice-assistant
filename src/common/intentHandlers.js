...
function handlePastSemestersTitleIntent(parameters) {
  ...
  let visualContent;
  if (!concert) {
    speechText = speechContents["NO_INFORMATION_FOR_PAST_SEMESTER"](
      semesterText,
      fullYear
    );
  } else {
...
function handleListPastSemestersTitlesIntent() {
  ...
  return {
    ...
    visualContent: visualContents["LIST_PAST_SEMESTERS_TITLES"](
      concerts,
      data.general
    ),
  };
...
function handleReservationInfoIntent() {
  ...
  return {
    ...
    visualContent: visualContents["RESERVATION_INFO"](
      data.general,
      nextConcert
    ),
  };
