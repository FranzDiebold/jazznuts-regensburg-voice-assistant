...
function listToText(list, conjunction='und') {
    ...
    if (! list || list.length === 0) {
        ...
        return `${list.slice(0, list.length - 1).join(', ')} ${conjunction} ${list.slice(-1)[0]}`.replace(/[^\w\säöüß,-]/gi, '');
    }
}
...
function getConcerts() {
    return data.concerts
        .map(concert => {
            ...
            concert.dates = concert.dates.sort((dateA, dateB) => (dateA < dateB ? -1 : 1));
            return concert;
        });
}
...
function getOrderedConcertsList() {
    return getConcerts()
        .sort((concertA, concertB) => (new Date(concertB.dates[0])) - (new Date(concertA.dates[0])));
}
...
function getNextConcert() {
    ...
    const now = new Date();
    const latestConcertLastDate = new Date(`${latestConcert.dates.slice(-1)[0]}T${latestConcert.time}:00+02:00`);
...
function getConcertsBySemesterAndYearMap() {
    return getConcerts()
        .reduce((concertsBySemesterAndYearMap, concert) => {
            concertsBySemesterAndYearMap[`${concert.semester}-${concert.year}`] = concert;
...
function getPastConcerts() {
    ...
    return getConcerts()
        .filter(concert => {
            const concertLastDate = new Date(`${concert.dates.slice(-1)[0]}T${concert.time}:00+02:00`);
...
function getConcertDaysText(concert) {
    return listToText(concert.dates
